...
V861 小核 RTOS 开发与大核是整合编译的，在编译大核时会加载小核的开发环境。
...
编译完成的固件会自动拷贝到对应板级的板级目录下，以供打包使用，例如这里的 perf2 板级的 RTOS 被拷贝到 `evice/config/chips/v861/configs/perf2/bin` 目录下，并且重命名为 `amp_rv0.bin`
