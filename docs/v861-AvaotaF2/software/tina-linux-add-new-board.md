...
这里将以 bga\_perf1 开发板，详解如何新建一个自己的板级方案 `v861-custom`

## 使用脚本新建板级方案

v861 Tina Linux 提供了新建板级脚本 `create_new_board`

1.  首先 `lunch` 作为基础的板级，这里执行 `lunch v861-bga_perf1-tina` 导入作为原型的板级
    
2.  然后执行命令 `create_new_board` 输入新建板子的名称，这里只需要输入名称即可，不需要带芯片型号，另外名称不支持 `-` 符号，支持下划线。这里输入 `custom` 即可创建 `v861-custom` 板级
...
前往`rtos/board/v861_e907`，将 `bga_perf1` 复制一份改名为 `custom`，这个文件夹内配置了方案RTOS的引脚文件，可以按需修改。
...
![image-20251226142500622](images/image-20251226142500622-7fd5f96770e371dc52a58ba3b0a607be.png)
