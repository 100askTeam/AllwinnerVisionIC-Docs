---
sidebar_position: 8
---

# SDK 开发环境配置

本文介绍如何在 Ubuntu 系统下安装 Tina Linux SDK 编译所需的依赖包。

完成环境配置后，您将具备：

-   编译 Tina Linux 源码的基础环境
-   打包固件所需的工具链支持

:::tip

:::note

虚拟机搭建

:::
:::note

如果您没有 Ubuntu 主机环境，可以使用虚拟机搭建 Ubuntu 开发环境。详见 [虚拟机搭建指南](./tina-linux-vbox.md)。

:::

:::
:::warning

:::note

架构限制

:::
:::note

**仅支持 x86\_64 (AMD64) 架构**，不支持 ARM 架构。

原因：Tina SDK 中的预编译工具链仅提供 x86\_64 版本。

:::

:::

## 快速概览

安装的依赖包主要用途：

| 类别 | 包名 | 用途 |
| --- | --- | --- |
| 编译基础 | `build-essential` | GCC、Make 等基础编译工具 |
| 代码管理 | `git`, `subversion` | 源码下载和版本管理 |
| 构建系统 | `quilt`, `flex`, `bison` | 补丁管理和语法解析 |
| 镜像打包 | `android-tools-mkbootimg` | Android boot.img 打包工具 |
| 32位支持 | `gcc-multilib`, `libc6:i386` | 编译 32 位兼容程序 |

## 编译系统支持

支持的 Ubuntu 版本：14、16、18、20、22、24、26

推荐使用 Ubuntu 22.04 或 24.04。

### Ubuntu 26.04

```shell
sudo apt-get update && \
for pkg in \
  build-essential \
  python3 \
  python3-dev \
  subversion \
  git \
  libncurses5-dev \
  zlib1g-dev \
  gawk \
  flex \
  bison \
  quilt \
  libssl-dev \
  xsltproc \
  libxml-parser-perl \
  mercurial \
  bzr \
  ecj \
  cvs \
  unzip \
  lsof \
  tree \
  python-is-python3 \
  kconfig-frontends \
  libpython3-dev \
  gcc-multilib \
; do sudo apt-get install -y "$pkg" || true; done && \
sudo dpkg --add-architecture i386 && sudo apt-get update && \
sudo apt-get install -y libc6:i386 libstdc++6:i386 lib32z1 || true
```

:::tip

:::note

安装失败处理

:::
:::note

脚本使用 `|| true` 处理失败，单个包不存在时自动跳过继续安装下一个。

如果某个必需包安装失败，可以手动执行：

```shell
sudo apt-get install -y <包名>
```

:::

:::

### Ubuntu 24.04

```shell
sudo apt-get update && \
for pkg in \
  build-essential \
  python3 \
  python3-dev \
  subversion \
  git \
  libncurses5-dev \
  zlib1g-dev \
  gawk \
  flex \
  bison \
  quilt \
  libssl-dev \
  xsltproc \
  libxml-parser-perl \
  mercurial \
  bzr \
  ecj \
  cvs \
  unzip \
  lsof \
  tree \
  python-is-python3 \
  kconfig-frontends \
  libpython3-dev \
  gcc-multilib \
; do sudo apt-get install -y "$pkg" || true; done && \
sudo dpkg --add-architecture i386 && sudo apt-get update && \
sudo apt-get install -y libc6:i386 libstdc++6:i386 lib32z1 || true
```

:::tip

:::note

安装失败处理

:::
:::note

脚本使用 `|| true` 处理失败，单个包不存在时自动跳过继续安装下一个。

:::

:::

### Ubuntu 22.04

```shell
sudo apt-get update && \
for pkg in \
  build-essential \
  python3 \
  python3-dev \
  subversion \
  git \
  libncurses5-dev \
  zlib1g-dev \
  gawk \
  flex \
  bison \
  quilt \
  libssl-dev \
  xsltproc \
  libxml-parser-perl \
  mercurial \
  bzr \
  ecj \
  cvs \
  unzip \
  lsof \
  tree \
  python-is-python3 \
  kconfig-frontends \
  libpython3-dev \
  gcc-multilib \
; do sudo apt-get install -y "$pkg" || true; done && \
sudo dpkg --add-architecture i386 && sudo apt-get update && \
sudo apt-get install -y libc6:i386 libstdc++6:i386 lib32z1 || true
```

**可选包**（镜像打包工具，缺失不影响编译）：

```shell
for pkg in \
  android-tools-mkbootimg \
  python2 \
; do sudo apt-get install -y "$pkg" || true; done
```

![更新软件源](images/image-20241121092718439-176672979806234-c2cfa8234d9ac7ea4e586bda1dc1a233.png)

![增加架构支持](images/image-20241121092739398-176672979806235-ecf40cb10d3b22857f7974cb983c86d0.png)

![安装32位支持包](images/image-20241121092825762-176672979806236-3cbcf8d1300bb734d8cde70a6f5b4f2a.png)

:::tip

:::note

安装失败处理

:::
:::note

脚本使用 `for` 循环逐包安装，`|| true` 确保单个包失败时不中断整个安装过程。

可选包（如 `android-tools-mkbootimg`、`python2`）缺失不影响核心编译功能。

:::

:::

### Ubuntu 20.04

```shell
sudo apt-get update && \
for pkg in \
  build-essential \
  python3 \
  python3-dev \
  subversion \
  git \
  libncurses5-dev \
  zlib1g-dev \
  gawk \
  flex \
  bison \
  quilt \
  libssl-dev \
  xsltproc \
  libxml-parser-perl \
  mercurial \
  bzr \
  ecj \
  cvs \
  unzip \
  lsof \
  tree \
  python-is-python3 \
  kconfig-frontends \
  libpython3-dev \
  gcc-multilib \
; do sudo apt-get install -y "$pkg" || true; done && \
sudo dpkg --add-architecture i386 && sudo apt-get update && \
sudo apt-get install -y libc6:i386 libstdc++6:i386 lib32z1 || true
```

**可选包**（镜像打包工具，缺失不影响编译）：

```shell
for pkg in \
  android-tools-mkbootimg \
  python2 \
; do sudo apt-get install -y "$pkg" || true; done
```

:::tip

:::note

安装失败处理

:::
:::note

脚本使用 `|| true` 处理失败，单个包不存在时自动跳过继续安装下一个。

:::

:::

### Ubuntu 18.04

**必需包**（编译核心依赖，必须安装）：

```shell
sudo apt-get update && \
for pkg in \
  build-essential \
  subversion \
  git \
  libncurses5-dev \
  zlib1g-dev \
  gawk \
  flex \
  bison \
  quilt \
  libssl-dev \
  xsltproc \
  libxml-parser-perl \
  mercurial \
  bzr \
  ecj \
  cvs \
  unzip \
  lsof \
  libpython3-dev \
  gcc-multilib \
; do sudo apt-get install -y "$pkg" || true; done && \
sudo dpkg --add-architecture i386 && sudo apt-get update && \
sudo apt-get install -y libc6:i386 libstdc++6:i386 lib32z1 || true
```

**可选包**（镜像打包工具，缺失不影响编译）：

```shell
sudo apt-get install -y android-tools-mkbootimg || true
```

:::tip

:::note

安装失败处理

:::
:::note

脚本使用 `|| true` 处理失败，单个包不存在时自动跳过继续安装下一个。

:::

:::

* * *

## WSL2 特殊配置

如果您使用 WSL2（Windows Subsystem for Linux），在完成上述 Ubuntu 配置后，还需进行以下额外配置。

### 清理环境变量

WSL2 会继承 Windows 的 PATH 环境变量，可能包含空格导致编译失败。

每次编译前执行：

```shell
export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/usr/local/games:/usr/lib/wsl/lib
```

### 启用 vsyscall 支持

WSL2 默认内核可能缺少 `vsyscall` 支持，导致部分编译工具链无法运行。

1.  打开 Windows 用户目录 `%userprofile%`

![打开用户目录](data:image/png;base64,)

2.  新建文件 `.wslconfig`

![新建配置文件](data:image/png;base64,)

3.  添加以下内容：

```ini
[wsl2]
kernelCommandLine = vsyscall=emulate
```

> 配置文件参考：[WSL 配置文档](https://docs.microsoft.com/en-us/windows/wsl/wsl-config#wslconfig)

4.  重启 WSL2：在 PowerShell 执行 `wsl --shutdown`，然后重新打开 WSL

:::note

:::note

WSL2 6.6.87.1 内核问题

:::
:::note

WSL2 内核版本 6.6.87.1 存在 BUG，无法启用 `vsyscall=emulate`。

-   问题追踪：[GitHub Issue #13031](https://github.com/microsoft/WSL/issues/13031)

![内核版本问题](images/image-20250608113617222-176672979806339-31b82558cbbc6351dc1027653550a81d.png)

解决方案：

-   更新 WSL2：`wsl --update`
-   或自行编译内核：[WSL2-Linux-Kernel](https://github.com/microsoft/WSL2-Linux-Kernel?tab=readme-ov-file#build-instructions)

:::

:::

* * *

## 验证环境配置

安装完成后，验证环境是否正确配置：

```shell
# 检查 GCC 版本
gcc --version

# 检查 Make 版本
make --version

# 检查 32 位支持
gcc -m32 --version
```

如果以上命令都能正常输出版本信息，说明环境配置成功。

* * *

## Windows USB 驱动安装

USB 驱动用于将编译好的固件烧写到开发板。如果您使用虚拟机，驱动应安装在 **宿主机 Windows** 上。

### 下载驱动

下载地址：[全志 USB 驱动](https://www.aw-ol.com/downloads/resources/15)

### 安装步骤

1.  解压下载的 `UsbDriver` 到任意文件夹

![解压驱动文件](images/image-20230329174037857-176672979806553-2f2037fd3f0d51817338ad7b730e0519.png)

2.  右键 **此电脑** → **管理**

![打开设备管理器](images/image-20230329174138747-176672979806554-cbe119b5ac658b04581536dd94ed6769.png)

3.  将开发板连接电脑并进入下载模式（按住 FEL 键上电）
    
4.  在设备管理器找到 `USB Device(VID_1f3a_PID_efe8)`
    

![识别USB设备](images/1648732462492-6fa8089e-5d2b-47f1-965f-9dcdf0c984da-image-176672979806555-a5c390f2603cc7fc82d55f04d8cc265a.png)

5.  右键该设备 → **更新驱动程序**

![更新驱动](images/1648732531793-f07e3def-dade-4069-8f1b-c4bbe2dc8078-image-176672979806556-ba8bdbdeaaa1493fa0d4825921a67554.png)

6.  选择 **浏览我的计算机查找驱动**

![浏览查找](images/1648732573134-f38b617c-b68d-4aea-abba-2cbdf5e9ae29-image-176672979806557-717b0d593df8b1a995cd254c2980ae3d.png)

7.  选择 **让我从可用驱动列表选取**

![从列表选取](images/1648732593215-f02c8901-55d1-47d2-ac13-fa3baf838fa8-image-176672979806558-36d3a015872763e53aae7192f8918731.png)

8.  点击 **从磁盘安装**

![从磁盘安装](images/1648732623019-43440003-9bbc-45d8-a4cd-36260a80bb0e-image-176672979806559-c60dfce0e9f3f4c3112704c649718457.png)

9.  点击 **浏览**，选择解压目录中的 `usbdrv.inf`

![选择驱动文件](images/1648732646080-2f99300d-cfd7-4aa4-b3aa-af1f893d3dae-image-176672979806560-0687f8afcfad1179557a3a7059054841.png)

![确认文件](images/1648732680712-8011100e-3e20-46a0-a4eb-c2bb372d6e6f-image-176672979806561-e5b9cfef0710d94a65dcbcc4cafbf17b.png)

10.  点击 **确定** → **下一步**

![确定安装](images/1648732725713-8b082aaf-20b3-4b7e-9a15-046a5db84e6a-image-176672979806562-4facbd7c3d4f3ea834e68a46244e2aba.png)

![下一步](images/1648732741539-d0928686-ccbc-4ca4-b5f1-54ffc1edde5c-image-176672979806663-e7727e8d374252a421cb9546bd5d97c4.png)

11.  安装完成，关闭窗口

![安装完成](images/1648732760111-c58ccd65-afd3-4bc9-b5f9-fd89f6b3ed66-image-176672979806664-721b0fb1fc0d11fca4323bb194170631.png)

### 常见问题：出现两个 USB Devices

![两个USB设备](images/1660320391935-6c485bb7-a1a2-4f6c-b275-3771fbb0f1ad-image-176672979806665-32a075baafad7a5e387c64d6c5f1b1dd.png)

解决方法：

1.  右键卸载多余的 USB Device
2.  勾选 **卸载驱动程序**

![卸载设备](data:image/png;base64,)

3.  重新安装驱动
