...
MIPI-DBI的类型如下，V821 支持的是 DBI TYPE C 协议，完整协议内容请参考《MIPI Alliance Standard for Display Bus Interface》：
...
开启功能会增加 CPU 占用，在 V821 平台将占用 4% CPU
...
SDK 默认会将 `openwrt/target/v821/v821-common/boot-resource/boot-resource/bootlogo.bmp` 进行 LZMA 压缩打包，所以正常使用需要勾选。
...
先准备一个 BMP 格式的图片，放到路径 `openwrt/target/v821/v821-common/boot-resource/boot-resource/bootlogo.bmp` 下
